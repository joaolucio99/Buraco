#include "../../../lib/pages/game_started/mechanics_edit_game.h"
    ...
    void on_button_edit_game_clicked(){     
    ...
    void on_edit_down_card_clicked(){      
        ...
        combo_box_text_hcards =  GTK_COMBO_BOX_TEXT( gtk_builder_get_object( builder, "card_to_add" ));
        dialog = GTK_WIDGET( gtk_builder_get_object( builder, "edit_game" ));
        int position_game = gtk_combo_box_get_active ( combo_box_games );       
        int choice_position = gtk_combo_box_get_active ( combo_box_position_choice );       
        int position_card_toadd = gtk_combo_box_get_active ( combo_box_hcards );       
            list_search_pos( player[active].hand, &card_to_add, position_card_toadd );  
            check_joker( position_game );      
    ...
    int check_card_to_down( cards card_to_add, int position_game, int choice, int position_card_toadd ){     
        if( choice == 0 ){ 
            if( card_to_add.joker == 1 ){   
                if( count_joker( position_game ) == 0 || card_to_add.number == player[active].games[position_game]->start->l_card.number-1 && card_to_add.suit == player[active].games[position_game]->start->l_card.suit ){   
                    ...
                    if( joker_position == 0 ){      
                        list_search_pos( player[active].games[position_game], &after_joker, joker_position+1);
                            if( card_to_add.number+1 == after_joker.number && card_to_add.suit == after_joker.suit || card_to_add.joker == 1 ){
                                ...
                                list_remove_pos( player[active].games[position_game], &temp_game, 0 );  
                                list_insert_pos( player[active].games[position_game] , temp_hand, 0 ); 
                                list_insert( player[active].hand, temp_game ); 
                            } else set_dialog( "CARTA INVÁLIDA PARA\nO JOGO ATUAL" );
                    }
                    else if( joker_position == list_lenght - 1 ) { 
    ...
    int count_joker( int position_game ){       
        header *aux = player[active].games[position_game]->start;
            while( aux != NULL ){
    ...
    void check_joker( int position_game ){         
        int count_list = list_count_size ( player[active].games[position_game], 0 );
        header *aux = player[active].games[position_game]->start;
        int count=0;
            while( aux != NULL && count < count_list-1 ){
                    if( aux->l_card.joker == 1 ){
                        if( aux->l_card.suit == aux->next->l_card.suit && aux->l_card.number+1 == aux->next->l_card.number || aux->next->l_card.joker == 1 ){       
                            if( count != 0 ){
                                    cards temp;
    ...
    void set_game_name_cb_box( char *game_name, int number, char*game_id ){    
    ...
    void on_edit_down_back_clicked(){   
        GtkWidget              *dialog;
        GtkComboBoxText        *combo_box_text_games;      
        GtkComboBoxText        *combo_box_text_hcards; 
        combo_box_text_games =  GTK_COMBO_BOX_TEXT( gtk_builder_get_object( builder, "game_to_edit" ));
